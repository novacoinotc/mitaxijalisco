      ...
      <header className="border-b border-white/10 glass">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2">
            ...
            <span className="ml-2 text-[10px] uppercase tracking-wider glass rounded-full px-2 py-0.5 text-white/60">{role}</span>
          </Link>
          <Link href="/" className="text-xs text-white/60 hover:text-white flex items-center gap-1"><Home className="h-3 w-3" /> Volver al inicio</Link>
        </div>
      </header>
      <div className="max-w-7xl mx-auto px-6 py-8 grid grid-cols-1 lg:grid-cols-[240px_1fr] gap-8">
        <aside className="space-y-1">
          {items.map(({ href, label, icon: Icon }) => {
            const active = p === href;
            return (
              <Link key={href} href={href} className={`flex items-center gap-3 rounded-xl px-4 py-3 text-sm ${active ? "bg-gradient-to-r from-neon-cyan/20 to-neon-violet/20 border border-neon-cyan/30 text-white" : "text-white/60 hover:bg-white/5 hover:text-white"}`}>
